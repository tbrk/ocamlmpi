...
let scatter data root comm =
  ...
  if myself = root then begin
    ...
    for i = 0 to nprocs - 1 do
      Bytes.blit_string buffers.(i) 0 send_buffer !pos lengths.(i);
      pos := !pos + lengths.(i)
    done;
    ...
    (* Do the scatter *)
    scatter_bytes (Bytes.create 0) [||] recv_buffer root comm;
...
let gather data root comm =
  ...
  if myself = root then begin
    ...
    (* Send our data *)
    gather_bytes send_buffer (Bytes.create 0) [||] root comm;
